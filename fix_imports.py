...
MODEL_SELECTION_REGEX = re.compile(
    r'from\s+["\']([^"\']*)model-selection(\.js)?["\'];?'
)
...
def process_file(file_path):
    ...
    for line in lines:
        if (
            "normalizeProviderId" in line
            and "model-selection" in line
            and "import" in line
        ):
            ...
            if match:
                ...
                if "normalizeProviderId" in imports:
                    ...
                    new_lines.append(
                        f'import {{ normalizeProviderId }} from "{provider_utils_path}";\n'
                    )
                    ...
                    if imports:
                        ...
                        new_lines.append(
                            f'import {{ {new_imports_str} }} from "{path_str}";\n'
                        )
